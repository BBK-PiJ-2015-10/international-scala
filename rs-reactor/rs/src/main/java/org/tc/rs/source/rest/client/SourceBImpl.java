...
@Component
public class SourceBImpl implements SourceB {
    ...
    @Autowired
    public SourceBImpl(@Value("${app.source.b.url}") String sourceRecordUrl, @Value("${app.source.b.port}") int sourceRecordPort,
                       @Value("${app.source.b.path}") String sourceRecordPath, SourceRecordMapper mapper) {
    ...
    public Mono<SourceRecord> fetchRecord() {
        return webClient.get().uri(sourceRecordURI).retrieve()
                ...
                .log()
                .map(this::mapXmlToRecord).flatMap(
                        optional -> optional.map(Mono::just).orElseGet(Mono::empty)
                );
    }

    private Optional<SourceRecord> mapXmlToRecord(String jsonResponse) {
        try {
            logger.info(String.format("Processing xmlResponse to %s", jsonResponse));
            var record = mapper.fromXML(jsonResponse);
            return record;
        } catch (Exception e) {
